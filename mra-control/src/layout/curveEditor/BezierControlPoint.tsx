...
export const BezierControlPoint = ({ id, point }: BezierControlPointProps) => {
  const [dragBase, setDragBase] = useState<THREE.Vector3 | undefined>(
    undefined
  );
  ...
  const setSelectedControlPoint = useCurveEditorState(
    (state) => state.setSelectedControlPoint
  );

  const updateBezierControlPoint = useCurveEditorState(
  ...
  return (
    <a.mesh {...spring} {...bind()} castShadow>
      <Sphere scale={0.2}>
        <meshBasicMaterial
          color={selectedPoint === id ? "#FF00FF" : "#000000"}
        />
      </Sphere>
    </a.mesh>
  );
